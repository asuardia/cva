module Contracts
    (Contract(..), Currency(..), Date(..), TimeStep(..), Obs(..),  
    mkDate,
    zero, one, give, cAnd, cOr, cond, scale, cWhen, anytime, cUntil,
    andGive, konst, lift, lift2, date, at, 
    european, american, between,
    PR(..), RV(..),
    takePr, horizonPr, andPr, 
    Pricer(..),  
    evalC, evalO, 
    bigK, konstSlices, datePr, timeSlices, condPr, liftPr, lift2Pr, lift2PrAll, lift3Pr, zipWithAll) 
where
...
evalC :: Pricer -> Currency -> Contract -> PR Double
evalC (Pricer disc exch snell absorb) k = eval    -- punning on record fieldnames for conciseness
